...
for i in tqdm(range(30)):
    ...
    outcome_scores = []
    for outcome in (possible_outcomes): # tqdmable
    ...
    # Get next day str
    nextday = test_data.iloc[test_data.shape[0]-1]['Datetime']
    date_format = '%Y-%m-%d %H:%M:%S%z'
    nextday = datetime.strptime(nextday,date_format)
    nextday = nextday + timedelta(days=1)
    nextdayStr = nextday.strftime('%Y-%m-%d %H:%M:%S%z')
...
for i,row in predictions.iterrows():
    sql = "INSERT INTO stockPredictionAAPL (" +cols + ") VALUES (" + "%s,"*(len(row)-1) + "%s)"
